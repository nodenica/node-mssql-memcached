...
exports.execute = function( procedure, inputs, connection, cache, lifetime, callback ){
    var parent = this;


    parent.out = function( paramProcedure, paramInputs, paramConnection, paramCache, paramLifetime, functionCallback ){
        ...
        if( paramCache ){
            ...
            paramInputs.forEach( function(obj) {
                inputString = inputString + obj.value;
            });

            // create a hash by query
            var key = getHash( paramProcedure + inputString );
                    ...
                    if( memcachedData ){
                        ...
                        var connection = new mssql.Connection( paramConnection, function( connectionError ){
                            if( connectionError ){
                                ...
                                paramInputs.forEach( function(obj) {
                                    request.input(obj.key, obj.type, obj.value);
                                });



                                request.execute( paramProcedure, function(executeError, recordsets, returnValue) {
                                    if( executeError ){

                                        if( debug ){
                                            console.log( 'Sql executeError: ' + executeError );
                                        }

                                        functionCallback( executeError, null );
                                    }
                                    else{
                                        memcached.set( key, recordsets, paramLifetime, function ( errSet ) {
                                            if( errSet ){
                                                ...
                                                functionCallback( null, recordsets );
    ...
    parent.out( procedure, inputs, connection, cache, lifetime, callback );
}
